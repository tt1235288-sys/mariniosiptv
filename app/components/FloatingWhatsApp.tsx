...
export default function FloatingWhatsApp() {
  ...
  // WhatsApp SVG Icon (for the floating button)
  const WhatsAppIcon = () => (
    <svg 
  ...
  // Girl Profile Picture Component
  ...
  return (
    <>
      {/* Auto Help Bubble - Single line only */}
      {!open && showBubble && (
      ...
      <button
        ...
        className="fixed bottom-6 right-6 z-[101] group"
      ...
      {/* Chat Window */}
      {open && (
        <div className="fixed bottom-24 right-4 md:right-6 w-[340px] sm:w-[380px] max-w-[calc(100vw-32px)] rounded-3xl overflow-hidden shadow-2xl z-[100] bg-white border border-slate-200 flex flex-col max-h-[80vh] md:max-h-[500px]">
          
          {/* Header - With Girl Profile Picture */}
          ...
          <div className="flex-1 p-4 flex flex-col gap-3 overflow-y-auto bg-slate-50 min-h-[200px] max-h-[300px] md:max-h-[350px]">
            {chatHistory.map((msg, idx) => (
              ...
              >
                {/* Support avatar on support messages */}
                {msg.sender === 'support' && (
